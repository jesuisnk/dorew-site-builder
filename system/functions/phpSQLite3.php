...
class phpSQLite3 extends \Twig\Extension\AbstractExtension
{
    ...
    {
        global $builder_site, $AuthorSite;
        $this->author = $AuthorSite;
        $this->builder_site = $builder_site;
        $this->core_cookie = $builder_site['default_login'];
        $path_sqlite = $builder_site['public_database'];
    ...
    public function getFunctions()
    {
        return [
            ...
            new \Twig\TwigFunction('system_info', [$this, 'system_info']),
    ...
    */
    ...
    /*
    -----------------------------------------------------------------
    SQL queries: get website information
    -----------------------------------------------------------------
    */

    function system_info($column = null)
    {
        if (!$column) {
            return 'There is not column in system_info()';
        } else {
            $ArrayColumn = ['domain', 'subdomain', 'reg'];
            $ArrayResult = [
                'domain' => $this->builder_site['domain'],
                'subdomain' => strtolower($this->author['subdomain']),
                'reg' => $this->author['reg']
            ];
            if (in_array($column, $ArrayColumn)) {
                return $ArrayResult[$column];
            } else {
                return 'The column ' . $column . ' is not exist in system_info()';
            }
        }
    }
