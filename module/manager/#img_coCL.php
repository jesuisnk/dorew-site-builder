...
if (is_login()) {
    ...
    //show image
    $get_file = $_GET['file'];
    $url_file = $builder_site['public_html'] . '/' . $get_file;
    ...
    if (file_exists($url_file) && is_file($url_file)) {
        ...
        if (in_array($ext, $ext_allow)) {
            ...
            $url_file = $protocol . $AuthorSite['subdomain'] . '.' . $system_domain . '/' . $get_file;
            echo '
        ...
        if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            if (!empty($file['name'])) {
                ...
                if (in_array($file_ext, $ext_allow) && $file_error == 0 && $file_size <= $maxSizeAllow) {
                    ...
                    $file_destination = $builder_site['public_html'] . '/' . $file_name_new;
                    if (move_uploaded_file($file_tmp, $file_destination)) {
                        echo '<div class="gmenu">Tải lên thành công - <b>' . file_size($file_size) . '</b></div>';
                        header('Refresh: 3; url=/cms/img.php');
                    } else {
